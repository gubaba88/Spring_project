...
<script type="text/javascript">
	function postDelete() {
		if (confirm("(${boardDTO.postNumber})번 글 \n(${boardDTO.postTitle})\n삭제 하시겠습니까?")) {
			location.href="./NoticeDelete?postNumber=${boardDTO.postNumber}"
		}
	}
</script>
</head>
...
<div class="container px-4 px-lg-5">
	...
	<c:choose>
		<c:when test="${empty sessionScope.memId}">
			<p align="center" style="margin-top: 16px"><a href="./Login">로그인</a>이 필요합니다.</p>
		</c:when>
		<c:when test="${sessionScope.memId == 'admin'}">
			<a href="./NoticeUpdate?&postNumber=${boardDTO.postNumber}"><button 
		...
		<c:otherwise>
			<p align="center" style="margin-top: 16px">공지사항 게시판의 글은 관리자만 수정, 삭제 할 수 있습니다.</p>
		</c:otherwise>
	</c:choose>
